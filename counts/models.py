...
class CountSheetManager(models.Manager):
    ...
    def ensure_period_sheet(
        ...
        *,
        ...
        # Frequency ke hisab se period ki start date calculate karo
        period_start = self._get_period_start(target_date, frequency)

        sheet, created = self.get_or_create(
            location=location,
            frequency=frequency,
            count_date=period_start,  # ← period ki start date use karo
    ...
    def _get_period_start(self, current_date: date, frequency: str) -> date:
        """Frequency ke hisab se period ki start date return kare"""
        if frequency == "mon_wed":
            # Current week ka Monday
            return current_date - relativedelta(days=current_date.weekday())
            ...
            # Bi-weekly: even/odd week based on year-week
            week_number = current_date.isocalendar()[1]
            year = current_date.year
            if week_number % 2 == 0:
                # Even week → previous Monday
                return current_date - relativedelta(days=current_date.weekday() + 7)
            else:
                return current_date - relativedelta(days=current_date.weekday())
            ...
            # Jan-June ya July-Dec
            if current_date.month <= 6:
                return current_date.replace(month=1, day=1)
            else:
                return current_date.replace(month=7, day=1)

        elif frequency == "annual":
            # Current year ki Jan 1
            return current_date.replace(month=1, day=1)
...
class CountEntry(models.Model):
    ...
    null=True,
    blank=True,
    related_name="created_count_entries",
)
    ...
    def perform_calculation(self) -> OrderCalculation:
        ...
        if on_hand <= order_point:
            ...
            highlight = self.HIGHLIGHT_GREEN
